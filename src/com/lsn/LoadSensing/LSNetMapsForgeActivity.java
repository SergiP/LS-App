...
public class LSNetMapsForgeActivity extends ActionBarMapOSMActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Drawable itemDefaultMarker = getResources().getDrawable(
				R.drawable.marker);
		itemizedOverlay = new LSNetworksOverlayForge(itemDefaultMarker,
				mapView, m_networks);
		...
		for (int i = 0; i < m_networks.size(); i++) {
			...
			String strNetDescripFormat = getResources().getString(
					R.string.strNetDescrip);
			String strNetDescrip = String.format(strNetDescripFormat,
					strSituation, strNumSensor);

			point = new GeoPoint(intLat, intLon);
			OverlayItem overlayItem = new OverlayItem(point, strName,
					strNetDescrip);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menuInflater.inflate(R.menu.ab_item_ov_config, menu);
